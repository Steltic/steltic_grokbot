<!-- chunk_id: send_p0 | collection: openseespy_documentation -->
<!-- meta: {
 "source": "openseespy",
 "source_file": "openseespy_documentation.md",
 "source_url": "https://openseespydoc.readthedocs.io/en/latest/src/send.html",
 "title": "11.4. send command",
 "category": "general",
 "command": "send",
 "doc_section": "src",
 "rel_path": "src/send.html",
 "part_index": 0,
 "part_count": 1,
 "char_count": 781,
 "word_count": 75,
 "has_code": false,
 "has_table": true
} -->

## 11.4. send command

**send(*'-pid'*, *pid*, **data*)**

Send information to another processor.

| `pid` ([int](https://docs.python.org/3/library/functions.html#int)) | ID of processor where data is sent to |
| --- | --- |
| `data` ([list](https://docs.python.org/3/library/stdtypes.html#list) ([int](https://docs.python.org/3/library/functions.html#int))) | can be a list of integers |
| `data` ([list](https://docs.python.org/3/library/stdtypes.html#list) ([float](https://docs.python.org/3/library/functions.html#float))) | can be a list of floats |
| `data` ([str](https://docs.python.org/3/library/stdtypes.html#str)) | can be a string |

Note

send command and [recv command](https://openseespydoc.readthedocs.io/en/latest/src/recv.html) must match and the order of calling both
commands matters.
